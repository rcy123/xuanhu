...
> 2026-07-22 沙盒范围注记：本文保留为未来真实临床轨道的历史/目标架构输入，不授权当前临床使用。个人学习工程沙盒不得把测试角色称为已认证医师；其当前权威设计使用 `Sandbox Reviewer`、显式测试身份/资质/签署字段、精确 challenge 绑定和一次性 resume，见 [L5 个人学习工程沙盒准入包](../L5个人学习工程沙盒准入包-2026-07-22.md)。本文中“interrupt 不可绕过”和“恢复后加载最新数据”的原表述不能单独作为沙盒验收证据；API/service/export、版本/digest、旧 review 失效和并发 CAS 必须分别验证。原历史决策内容不删除、不重写。

## 状态

已采纳（2026-07-09；原医疗/Doctor 语义仅作为未来临床轨道历史目标，个人学习沙盒部分已由 2026-07-22 沙盒准入包取代）
