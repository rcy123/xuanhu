...
class Settings(BaseSettings):
    ...
    # -------------------------------------------------------------------
    # 设置后，FastAPI 直接托管 Vite 构建产物（frontend/dist）：
    #   - /assets/* 静态文件由 StaticFiles 提供
    #   - 其余非 API 路径回退 index.html（SPA 客户端路由）
    # 生产多进程/高可用场景仍推荐 Nginx 同源反代（deploy/nginx/xuanhu.conf），
    # 此开关面向单进程本地部署与演示环境。
    ui_dist_dir: str | None = Field(
        default=None,
        validation_alias="XUANHU_UI_DIST",
        description="前端构建产物目录（如 frontend/dist）；为空则不托管前端静态文件",
    )
