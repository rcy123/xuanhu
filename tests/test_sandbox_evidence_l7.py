...
# ---------------------------------------------------------------------------
# The module under test — RED until implemented
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Fixed synthetic fixtures — deterministic, no real data
# ---------------------------------------------------------------------------
...
# ===================================================================
# 2. Module is importable (RED first — module does not exist yet)
# ===================================================================


def test_l7_module_can_be_imported() -> None:
    """If this fails, sandbox_evidence.py does not exist — expected RED first."""
...
class TestEvidenceStore:
    ...
    def test_same_key_same_bytes_is_idempotent(self) -> None:
        ...
        bundle = _fake_bundle()
        store.put(bundle)
        store.put(bundle)  # second put should succeed
    ...
    def test_same_key_different_bytes_is_rejected(self) -> None:
        ...
        # Store validates that retrieved bytes match what was stored
        # (corruption detection is covered by tampered snapshot test)
    ...
    def test_snapshot_roundtrip(self) -> None:
        ...
        restored = SandboxEvidenceStore.restore(snapshot)
    ...
    def test_snapshot_size_limit(self) -> None:
        ...
        for i in range(200):
            ...
            store.put(b)
        snapshot = store.snapshot()
        # Serialize once more — must stay under limit
    ...
    def test_restore_deterministic_replay(self) -> None:
        ...
        store_b = SandboxEvidenceStore()
        store_b.put(_fake_bundle(query="q-before", retrieval_run="rr-before"))
        store_b = SandboxEvidenceStore.restore(snap_a)

        assert store_b.get(b1.bundle_digest) is not None
        assert store_b.get(b2.bundle_digest) is not None
        # q-before should be gone
        assert store_b.get(_fake_bundle(query="q-before", retrieval_run="rr-before").bundle_digest) is None
    ...
    def test_restore_rejects_tampered_snapshot(self) -> None:
        ...
        store.put(bundle)
        snap = store.snapshot()
        # Tamper the data — the model no longer auto-validates digest,
        # but restore must catch it
        tampered = SandboxEvidenceStoreSnapshotV1(
            data=snap.data + "tampered",
            digest=snap.digest,
        )
        with pytest.raises(SandboxEvidenceError):
            SandboxEvidenceStore.restore(tampered)

    def test_store_rejects_self_recompute_snapshot(self) -> None:
        """A snapshot cannot be its own authorizer — re-authorize after restore."""
        store = SandboxEvidenceStore()
        b1 = _fake_bundle()
        store.put(b1)
        snap = store.snapshot()
...
class TestCitationVerifier:
    ...
    def test_wrong_source_type_rejected(self) -> None:
        verifier = CitationVerifier()
        # Syndrome basis with a FORMULA source — should fail
...
# ===================================================================
# 9. EvidencePipeline (retrieve → store → claim link → verify → context)
# ===================================================================
...
class TestThreatModel:
    ...
    def test_enum_scalar_substance_rejected(self) -> None:
        ...
        with pytest.raises(ValidationError):
            SandboxEvidencePacketV1(
                evidence_id="ev-001",
                source_type="theory",  # not a SourceType enum — strict rejects
                source_id="src-1",
                chunk_id="chunk-1",
                rank=1,
                content_digest="a" * 64,
    ...
    def test_callback_reentry_during_authorize(self) -> None:
        ...
        class ReentrantAuthorizer:
            def recognize(self, *, bundle: object) -> bool:
                return True

            def authorize(self, *, bundle: object) -> bool:
                entered.set()
                # Inside authorize, try a store operation
                try:
        ...
        auth = ReentrantAuthorizer()
        _ = auth  # keep reference for reentry probe
        # Use the store's verify mechanism that invokes callbacks
        # Callback wraps the auth
        thread = threading.Thread(
            target=lambda: store.put(
                _fake_bundle(query="reentry", retrieval_run="rr-reentry"),
            )
        )
        thread.start()
        entered.wait(timeout=5)
        # While in authorize, store should be consistent
        released.set()
    ...
    def test_authority_revoked_after_use_fails_closed(self) -> None:
        ...
        b1 = _fake_bundle()
        store.put(b1)
        assert store.get(b1.bundle_digest) is not None
        # Revocation is implicit via restoring a fresh empty store
        empty = SandboxEvidenceStore()
        assert empty.get(b1.bundle_digest) is None
